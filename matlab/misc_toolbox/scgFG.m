function [x, options, flog, pointlog, scalelog] = scg(fgradf, x, options, varargin)
...
fgradf = fcnchk(fgradf, length(varargin));
...
sigma0 = 1.0e-4;
[fold, gradnew] = feval(fgradf, x, varargin{:});	% Initial function value.
gradnew = gradnew';
fnow = fold;
...
while (j <= niters)
  ...
  if (success == 1)
    ...
    xplus = x + sigma*d;
    [ftmp, gplus] = feval(fgradf, xplus, varargin{:});
    gplus = gplus';
  ...
  xnew = x + alpha*d;
  fnew = feval(fgradf, xnew, varargin{:});
  ...
  if (success == 1)
    ...
    if (max(abs(alpha*d)) < options(2) & max(abs(fnew-fold)) < options(3))
      ...
      fold = fnew;
      gradold = gradnew;
      [ftmp, gradnew] = feval(fgradf, x, varargin{:});
      gradnew = gradnew';
      options(11) = options(11) + 1;
