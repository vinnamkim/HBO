...
def Lower_bound(N, K, D, M, X, y, Z, mu, Sigma, sigma_f, sigma):
    ...
    div = tf.transpose(tf.expand_dims(tf.matmul(tf.square(X), tf.transpose(Sigma)), 0)) # K x N x 1
    ...
    Psi1 = sigma_f * tf.reduce_prod(tf.rsqrt(div_plusone) * tf.exp(- tf.square(mu_x - ZZ) / (2 * div_plusone)), axis = 0) # K x N x M
    # tf.square(mu_x - ZZ) Check
    # tf.exp(- tf.square(mu_x - ZZ) / (2 * div_plusone)) Check
    # tf.rsqrt(div_plusone) Check
    # tf.rsqrt(div_plusone) * tf.exp(- tf.square(mu_x - ZZ) / (2 * div_plusone)) Check
    # Psi1 Check
    ...
    Psi21 = tf.square(sigma_f) * tf.exp(-0.25 * tf.reduce_sum(tf.square(ZZ - ZZ_t), axis = 0)) # Check
    # Psi21 Check
    ...
    Psi2 = Psi21 * Psi22
    # tf.square(mu_xx - ZZZ) Check
    # tf.rsqrt(twodiv_plusone) * tf.exp(-tf.square(mu_xx - ZZZ) / twodiv_plusone Check 
    # Psi22 Check
    
    sigma_sq = tf.square(sigma)
    A = tf.cholesky(sigma_sq * K_uu + Psi2)
    C = 0.5 / sigma_sq
    N_ = tf.constant(N, dtype = FLOATING_TYPE)
    M_ = tf.constant(M, dtype = FLOATING_TYPE)
    pi_ = tf.constant(np.pi, dtype = FLOATING_TYPE)
    y_Psi1 = tf.matmul(tf.transpose(y), Psi1)
    
    F1 = tf.reduce_sum(- N_ * 0.5 * tf.log(2 * pi_) - \
    0.5 * (N_ - M_) * tf.log(sigma_sq) + \
    0.5 * tf.reduce_sum(tf.log(tf.diag_part(K_uu_chol))) - \
    0.5 * tf.reduce_sum(tf.log(tf.diag_part(A))) - \
    C * tf.matmul(tf.transpose(y), y) + \
    C * tf.matmul(y_Psi1, tf.cholesky_solve(A, tf.transpose(y_Psi1))) - \
    C * N_ * tf.square(sigma_f) + \
    C * tf.trace(tf.cholesky_solve(K_uu_chol, Psi2))) 
    
    D_ = tf.constant(D, dtype = FLOATING_TYPE)
    
    KL = 0.5 * tf.reduce_sum(tf.reduce_sum(tf.log(Sigma), 1) - D_ * tf.log(tf.reduce_sum(tf.square(mu) + Sigma, 1)) + D_ * tf.log(D_)) # Check
        
    l_square = tf.reduce_sum(tf.square(mu) + Sigma, 1) / D_
...
def GP_train(N, K, D, M, X, y, Z, mu, Sigma, sigma_f, sigma, barrier_coeff, learning_rate):
